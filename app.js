...
// Populate item datalist with tier 4-8 weapons/armor examples
const itemExamples = [
...
async function fetchPrices(ids, city){
  ...
  const url = `https://west.albion-online-data.com/api/v2/stats/prices/${joined}.json?locations=${city}&qualities=1`;
...
document.getElementById('calcBtn').addEventListener('click', async ()=>{
  ...
  if(!recipe || !recipe.craftingrequirements || !recipe.craftingrequirements.craftresource){
  ...
  const matMap = new Map(prices.filter(p=>p.sell_price_min>0).map(p=>[p.item_id, p.sell_price_min]));
  const itemPrice = prices.find(p=>p.item_id===itemId)?.sell_price_min || 0;
  ...
  document.getElementById('sellPrice').textContent = (itemPrice*qty).toLocaleString();
  document.getElementById('profit').textContent = ((itemPrice*qty)-totalMatCost).toLocaleString();
